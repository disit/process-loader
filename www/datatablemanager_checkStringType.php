...
for ($sheetIndex = 0; $sheetIndex < count($sheetNames); $sheetIndex ++) {
    ...
    for ($rowIndex = 1; $rowIndex < count($sheet_rows_data); $rowIndex ++) {
        ...
        for ($cell_number = 0; $cell_number < count($row); $cell_number ++) {
            if ($cell_number == $col_index && $row[$cell_number]!="" &&((is_float($row[$cell_number]))||(is_numeric($row[$cell_number])))) {
                $result='false';
                    break 3;
                }
            }
