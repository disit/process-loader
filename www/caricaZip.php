...
$descrizione_test = mysqli_real_escape_string($connessione_al_server,$_POST['filedesc']);
$descrizione = filter_var($descrizione_test, FILTER_SANITIZE_STRING);
//
...
//
$subnat_test = mysqli_real_escape_string($connessione_al_server,$_POST['filesubN']);
$subnat = filter_var($subnat_test , FILTER_SANITIZE_STRING);
// 
...
//CheckType//
if (($tipo !== "ETL") && ($tipo !== "R") && ($tipo !== "Java") && ($tipo !== "IoTApp") && ($tipo !== "IoTBlocks") && ($tipo !== "AMMA") && ($tipo !== "ResDash") && ($tipo !== "DevDash") && ($tipo !== "MicroService") && ($tipo !== "ControlRoomDashboard") && ($tipo !== "Mobile App") && ($tipo !=="DataAnalyticMicroService")) {
    ...
    if ($tipo == 'MicroService') {
        ...
        if (isset($_POST['paramList'])) {
            //$array_parametri = mysqli_real_escape_string($connessione_al_server,$_POST['paramList']);
        ...
        if (isset($_POST['micro_param'])) {
            $string_value0 = mysqli_real_escape_string($connessione_al_server,$_POST['micro_param']);
			$string_value = filter_var($string_value0 , FILTER_SANITIZE_STRING);
			//
        ...
        if (isset($_POST['micro_user'])) {
            $micro_user0 = mysqli_real_escape_string($connessione_al_server,$_POST['micro_user']);
			$micro_user = filter_var($micro_user0 , FILTER_SANITIZE_STRING);
        } else {
            $micro_user = "";
        }
        
        if (isset($_POST['micro_pass'])) {
            $micro_pass0 = mysqli_real_escape_string($connessione_al_server,$_POST['micro_pass']);
			$micro_pass = filter_var($micro_pass0 , FILTER_SANITIZE_STRING);
        } else {
            $micro_pass = "";
        }
